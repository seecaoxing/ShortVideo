...
public class AES {
    ...
    private static final String keyBytes = "neteaseradishent";
    ...
    public static byte[] decode(byte[] content) {
        return decrypt(content, keyBytes);
    }
    ...
    public static String encodeStr(String content, String key){
        ...
        String encodeStr = "";
        try {
            encodeStr = parseByte2HexStr(encrypt(content.getBytes("UTF-8"),AES.parseHexStr2Byte(key)));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return encodeStr;
